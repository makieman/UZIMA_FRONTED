...
// Generate Kenya timestamp (UTC+3) - exactly 14 digits
function generateKenyaTimestamp(): string {
  const now = new Date();
  // Convert to Kenya time (UTC+3)
  const kenyaTime = new Date(now.getTime() + (3 * 60 * 60 * 1000));
  
  // Format manually to ensure exactly 14 digits (without padStart for compatibility)
  const year = kenyaTime.getFullYear();
  const month = (kenyaTime.getMonth() + 1).toString().length === 1 ? '0' + (kenyaTime.getMonth() + 1) : (kenyaTime.getMonth() + 1).toString();
  const day = kenyaTime.getDate().toString().length === 1 ? '0' + kenyaTime.getDate() : kenyaTime.getDate().toString();
  const hours = kenyaTime.getHours().toString().length === 1 ? '0' + kenyaTime.getHours() : kenyaTime.getHours().toString();
  const minutes = kenyaTime.getMinutes().toString().length === 1 ? '0' + kenyaTime.getMinutes() : kenyaTime.getMinutes().toString();
  const seconds = kenyaTime.getSeconds().toString().length === 1 ? '0' + kenyaTime.getSeconds() : kenyaTime.getSeconds().toString();
